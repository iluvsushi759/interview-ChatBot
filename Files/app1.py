...
if prompt := st.chat_input("Your answer."):   # := assigns the input to prompt and checks if it's not empty
    ...
    with st.chat_message("assistant"):
        stream = client.chat.completions.create(
            ...
            messages=[
                ...
                for m in st.session_state.messages   # sending the entire conversation history to the model to maintain context.  You should do some trimming / summarizing (token management) in a production app to avoid hitting token limits
            ],
